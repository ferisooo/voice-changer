...
class VoiceChangerManager(ServerAudioCallbacks):
    ...
    def __init__(self):
        ...
        try:
            with open(STORED_SETTING_FILE, "r", encoding="utf-8") as f:
                self._stored_settings = json.load(f)
            self.settings.set_properties(self._stored_settings)
        except:
            pass
        ...
        # Re-apply the user's saved settings so the model's per-slot defaults
        # (e.g. protect=0.5, tran, indexRatio) don't overwrite them on launch.
        if self._stored_settings:
            try:
                self.settings.set_properties(self._stored_settings)
    ...
    def get_auto_pitch_status(self) -> dict:
        model = getattr(self.vc, "vcmodel", None)
        if model is None or not hasattr(model, "get_auto_pitch_status"):
            return {"enabled": False, "available": False}
        status = model.get_auto_pitch_status()
        status["available"] = True
        return status

    def get_auto_smooth_status(self) -> dict:
        try:
            if self.vc is None or self.vc.vcmodel is None:
                return {"enabled": False, "available": False}
            status = self.vc.get_auto_smooth_status()
            status["available"] = True
            return status
        except Exception:
            return {"enabled": False, "available": False}
    ...
    def start_calibration(self) -> dict:
        model = getattr(self.vc, "vcmodel", None)
        if model is None or not hasattr(model, "start_calibration"):
            return {"active": False, "available": False}
        model.start_calibration()
        self._cal_saved = False
        return self.get_calibration_status()

    def get_calibration_status(self) -> dict:
        model = getattr(self.vc, "vcmodel", None)
        if model is None or not hasattr(model, "get_calibration_status"):
            return {"active": False, "available": False}
        status = model.get_calibration_status()
        status["available"] = True
        # Persist the new profile to disk once, when it becomes ready.
        if status.get("ready") and not getattr(self, "_cal_saved", False):
            try:
                self.store_setting()
            except Exception as e:
                logger.exception(e)
            self._cal_saved = True
        return status
